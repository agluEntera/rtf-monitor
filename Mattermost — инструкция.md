...
После создания **первой** команды скопировать **Token** и вставить в `.env` как `MATTERMOST_TOKEN`.
Остальные команды создавать с тем же токеном — или оставить `MATTERMOST_TOKEN` пустым, чтобы отключить проверку.
